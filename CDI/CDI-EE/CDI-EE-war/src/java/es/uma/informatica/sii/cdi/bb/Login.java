...
public class Login {
    ...
    public String autenticar() {
        ...
        if(this.usuario.length()==0)ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Introduzca el usuario", "Introduzca el usuario"));
        else {
        	Usuario actual = null;
        	for(Usuario user: usuarios) {
        		if(user.equals(new Usuario(this.usuario, this.password)))actual=user;
        	}
        	if(actual==null)ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "El usuario no existe", "El usuario no existe"));
        	else if(!password.contentEquals(actual.getPassword()))ctx.addMessage(null, new FacesMessage(FacesMessage.SEVERITY_ERROR, "Contrasenia incorrecta", "Contrasenia incorrecta"));
        	else {
